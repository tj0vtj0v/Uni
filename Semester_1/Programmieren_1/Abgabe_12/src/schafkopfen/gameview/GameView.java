...
public class GameView {
    ...
    private static void enforceLowerCaseLettersForResourceFileName(String resourceFileName) {
        if (!resourceFileName.toLowerCase().equals(resourceFileName)) {
            throw new InputMismatchException("Only lower case letters are allowed for resource files: " + resourceFileName);
        }
    }
     ...
     */
    public void setMouseCursor(String cursorImageFileName, boolean centered) {
    ...
    private static class Mouse implements ActionListener {
        ...
        void setMouseCursor(String cursorImageFileName, boolean centered) {
            this.invisibleMouseCursor = false;
    ...
    private static class Sound {
        ...
        private void readSoundBytesAndStoreAsBytesAndClip(String soundFile) {
            if (!storedSoundBytes.containsKey(soundFile)) {
                enforceLowerCaseLettersForResourceFileName(soundFile);
    ...
    private static class SwingAdapter {
        ...
        BufferedImage createImageFromFile(String imageFileName) {
            ...
            if (image == null) {
                try {
                    enforceLowerCaseLettersForResourceFileName(imageFileName);
                    image = ImageIO.read(Objects.requireNonNull(GameView.class.getResource("/resources/" + imageFileName)));
                } catch (Exception e) {
                    e.printStackTrace();
        ...
        void setWindowIcon(String iconFileName) {
            ...
            try {
                enforceLowerCaseLettersForResourceFileName(iconFileName);
        ...
        void setMouseCursor(String cursorImageFileName, boolean centered) {
            try {
                enforceLowerCaseLettersForResourceFileName(cursorImageFileName);
    ...
    private static class Version {
        private final static String LAST_CHANGE = "2022-11-08";
